...
describe("Can path go to node", () => {
  it("From any direction, path can continue by stepping on an end node", () => {
    expect(canPathGoToNode(getNodeWithValue(END_VALUE), Direction.Up)).toBe(
      true
    );
    expect(canPathGoToNode(getNodeWithValue(END_VALUE), Direction.Down)).toBe(
      true
    );
    expect(canPathGoToNode(getNodeWithValue(END_VALUE), Direction.Right)).toBe(
      true
    );
    expect(canPathGoToNode(getNodeWithValue(END_VALUE), Direction.Left)).toBe(
      true
    );
  });
  ...
  it("From any direction, path can continue by stepping on a turn node", () => {
    expect(canPathGoToNode(getNodeWithValue(TURN_VALUE), Direction.Up)).toBe(
      true
    );
    expect(canPathGoToNode(getNodeWithValue(TURN_VALUE), Direction.Down)).toBe(
      true
    );
    expect(canPathGoToNode(getNodeWithValue(TURN_VALUE), Direction.Right)).toBe(
      true
    );
    expect(canPathGoToNode(getNodeWithValue(TURN_VALUE), Direction.Left)).toBe(
      true
    );
  });
  ...
  it("From vertical directions, path can continue by stepping on a vertical path node", () => {
    expect(
      canPathGoToNode(getNodeWithValue(VERTICAL_VALUE), Direction.Up)
    ).toBe(true);
    expect(
      canPathGoToNode(getNodeWithValue(VERTICAL_VALUE), Direction.Down)
    ).toBe(true);
  });

  it("From vertical directions, path can not continue by stepping on a horizontal path node", () => {
    expect(
      canPathGoToNode(getNodeWithValue(HORIZONTAL_VALUE), Direction.Up)
    ).toBe(false);
    expect(
      canPathGoToNode(getNodeWithValue(HORIZONTAL_VALUE), Direction.Down)
    ).toBe(false);
  });

  it("From horizontal directions, path can continue by stepping on a horizontal path node", () => {
    expect(
      canPathGoToNode(getNodeWithValue(HORIZONTAL_VALUE), Direction.Right)
    ).toBe(true);
    expect(
      canPathGoToNode(getNodeWithValue(HORIZONTAL_VALUE), Direction.Left)
    ).toBe(true);
  });

  it("From horizontal directions, path can not continue by stepping on a vertical path node", () => {
    expect(
      canPathGoToNode(getNodeWithValue(VERTICAL_VALUE), Direction.Right)
    ).toBe(false);
    expect(
      canPathGoToNode(getNodeWithValue(VERTICAL_VALUE), Direction.Left)
    ).toBe(false);
  });
});
...
describe("Get node error message", () => {
  it("Should construct correct message for inputs", () => {
    ...
    expect(createNodeErrorMsg(node, "Something bad happened")).toBe(
      "Invalid A node at [0][0]\nSomething bad happened"
    );

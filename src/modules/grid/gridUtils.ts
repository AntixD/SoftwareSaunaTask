import { PathElement, PathGrid, VisitedGrid } from "../../types/types";
import { Coordinates, Node } from "../../types/interfaces";
import { getCoordinatesForDirection } from "../direction/directionUtils";
...
const findCoordinatesOfElement = (
  pathGrid: PathGrid,
  el: PathElement
...
export const getNeighbourNodeCoordinates = (
  node: Node,
  direction: Direction
): Coordinates => ({
  row: node.coordinates.row + getCoordinatesForDirection(direction).row,
  column:
    node.coordinates.column + getCoordinatesForDirection(direction).column,
});

export const hasNodeBeenVisited = (
  visitedGrid: VisitedGrid,
  node: Node
): boolean => visitedGrid[node.coordinates.row][node.coordinates.column];

export const findFirstNodeWithElement = (
  pathGrid: PathGrid,
  value: PathElement
): Node | undefined => {

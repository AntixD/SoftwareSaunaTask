import { ALL_DIRECTIONS, START_VALUE } from "../../constants/constants";
import { PathGrid, VisitedGrid } from "../../types/types";
import { Coordinates, FollowPathResult, Node } from "../../types/interfaces";
...
import {
  ...
  isStartNode,
  isTurnNode,
} from "../../modules/node/nodeUtils";
import { getAllowedDirectionsOnTurn } from "../../modules/direction/directionUtils";

const getNextNodeInDirection = (
  pathGrid: PathGrid,
  node: Node,
  direction: Direction
): Node | undefined => {
  const neighbourCoordinates: Coordinates = getNeighbourNodeCoordinates(
    node,
    direction
  );
...
const getNextMoveForMultipleDirections = (
  ...
  const possibleMoves = allowedDirections.filter((dir) =>
    canPathMoveInDirection(pathGrid, node, dir, visitedGrid)
  );
  if (possibleMoves.length === 1) {
    ...
    const errorDetails =
      possibleMoves.length > 1
        ...
        : "There are no possible allowed moves.";
...
const getNextValidMoveDirection = (
  pathGrid: PathGrid,
  visitedGrid: VisitedGrid,
  node: Node,
  direction: Direction
  ...
  if (isStartNode(node)) {
    return getNextMoveForMultipleDirections(
      pathGrid,
      node,
      visitedGrid,
      ALL_DIRECTIONS
  ...
  if (hasNodeBeenVisited(visitedGrid, node) || isBasicPathNode(node)) {
    return getNextMoveForSingleDirection(
      pathGrid,
      node,
      visitedGrid,
      direction
  ...
  if (isTurnNode(node)) {
    return getNextMoveForMultipleDirections(
      ...
      node,
      visitedGrid,
      getAllowedDirectionsOnTurn(direction)
    );
  }
  ...
  throw new Error(
    createNodeErrorMsg(node, "Unknown node value, cannot determine next move.")
  );
};
...
const traverseNode = (
  ...
  const nextValidMoveDirection = getNextValidMoveDirection(
    pathGrid,
    visitedGrid,
    node,
    direction
  );
  const nextNode = getNextNodeInDirection(
    pathGrid,
    node,
    nextValidMoveDirection
  );
  if (nextNode == null) {
    throw new Error(
      createNodeErrorMsg(
        node,
        `Should go in direction: ${direction}, but it is not possible.`
      )
    );
  } else {
